...
object S3DataFetcher {
  ...
  def getEvents(query: Query): Array[String] = {
    getFilePaths(query.bucket.get, query.prefix.get, query.startDate, query.endDate, query.datePattern.getOrElse("yyyy-MM-dd")).filterNot { x => x.isEmpty() }
  }

  def getFilePaths(bucketName: String, prefix: String, fromDate: Option[String] = None, toDate: Option[String] = None, pattern: String = "yyyy-MM-dd"): Array[String] = {
    val from = fromDate
    if (from.nonEmpty) {
