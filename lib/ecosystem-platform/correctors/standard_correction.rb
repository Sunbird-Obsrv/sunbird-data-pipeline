...
module EcosystemPlatform
  module Correctors
    class StandardCorrection

      PROGNAME = 'standard_correction.jobs.ep'
      EVENTS_N = 10000
      SLICE = 10000
          ...
          loop do
            ...
            response = @client.search({
              ...
              body: {
                "from" => offset,
                "size" => EVENTS_N,
                "query" => {"filtered": {"filter": {"bool": {"should": [{"exists": {"field": "udata"}}],"must": [{"missing": {"field": "udata.standard"}}]}}}}
              }
            })
            response = Hashie::Mash.new response
            logger.info "PAGE: #{page} - FOUND #{response.hits.hits.count} hits. - TOTAL #{response.hits.total}"
            response.hits.hits.each do |hit|
              standard = hit._source.udata.standard
              if(standard == nil)
