...
public class DeDuplicationTask implements StreamTask, InitableTask, WindowableTask {
	...
	private void init(Config config, TaskContext context,
		...
		deDupEngine = deDupEngine == null ?
				new DeDupEngine(new Client(config.get("redis.host", "localhost"),config.getInt("redis.port", 6379)
						,config.getInt("redis.connection.idle.max",20),config.getInt("redis.connection.max",4))) : deDupEngine;
