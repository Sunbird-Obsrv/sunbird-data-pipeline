...
public class Event implements Mappable {
	...
	public String getDid() {
		...
		return did.isNull() ? telemetry.<String>read(path.contextDid()).value() : did.value();
	}
	...
	public String getObjectID(Map<String, Object> contentTaxonomy) {
		...
		for (String event : contentTaxonomy.keySet()) {
			if (getEid().startsWith(event.toUpperCase())) {
				ArrayList<String> fields = getRemovableFields(event, contentTaxonomy);
	...
	private ArrayList<String> getRemovableFields(String event, Map<String, Object> contentTaxonomy) {
	...
	private boolean objectFieldsPresent() {
