...
public class TaxonomyStreamTask implements StreamTask, InitableTask {
    ...
    public void processEvent(Event event, MessageCollector collector) throws Exception {
        ...
        Map<String, Object> taxonomyData = null;

        try {
            if (taxonomyStore.get(cid) == null) {
                System.out.println("if taxonomy store is null");
                taxonomyData = (Map<String, Object>) getTaxonomyLibrary();
                System.out.println("taxonomyData"+taxonomyData);
                createTaxonomyHash(taxonomyData, taxonomyStore);
            ...
            System.out.println("Taxonomy Events"+event.getMap());
