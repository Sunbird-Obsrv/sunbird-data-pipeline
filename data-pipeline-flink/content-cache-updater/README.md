...
This job is used to update content and dialcode metadata to  redis by running the job using Flink. It can be run either on Yarn or Kubernetes.
...
```
./bin/flink run -m yarn-cluster -p 2 -yjm 1024m -ytm 1024m <data-pipeline-src-dir>/content-cache-updater/target/content-cache-updater-0.0.1.jar
```
...
./bin/flink run -m localhost:8081 <data-pipeline-src-dir>/content-cache-updater/target/content-cache-updater-0.0.1.jar
