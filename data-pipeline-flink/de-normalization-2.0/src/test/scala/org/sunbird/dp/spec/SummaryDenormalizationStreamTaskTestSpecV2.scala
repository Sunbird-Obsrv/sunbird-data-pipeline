...
import org.sunbird.dp.denorm.task.{DenormalizationConfigV2, SummaryDenormalizationStreamTaskV2}
...
class SummaryDenormalizationStreamTaskTestSpecV2 extends BaseTestSpec {
  ...
  val denormConfig: DenormalizationConfigV2 = new DenormalizationConfigV2(config, "SummaryDenormTest")
  ...
  "Summary Denormalization pipeline" should "denormalize content, user, device and location metadata for summary events" in {
    ...
    val task = new SummaryDenormalizationStreamTaskV2(denormConfig, mockKafkaUtil)
    task.process()
