...
import org.sunbird.dp.denorm.functions.DenormalizationFunctionV2
import org.sunbird.dp.core.job.FlinkKafkaConnector
import org.sunbird.dp.core.util.FlinkUtil
import org.sunbird.dp.denorm.functions.SummaryDeduplicationFunctionV2
...
/**
  ...
  */
class SummaryDenormalizationStreamTaskV2(config: DenormalizationConfigV2, kafkaConnector: FlinkKafkaConnector) {
  ...
  def process(): Unit = {
    ...
    val summaryEventStream =
      env.addSource(source, config.summaryDenormalizationConsumer).uid(config.summaryDenormalizationConsumer)
        ...
        .process(new SummaryDeduplicationFunctionV2(config)).name(config.summaryDedupFunction).uid(config.summaryDedupFunction)
    ...
    val summaryDenormStream = summaryEventStream.getSideOutput(config.uniqueSummaryEventsOutputTag)
      .process(new DenormalizationFunctionV2(config))
...
object SummaryDenormalizationStreamTaskV2 {

  def main(args: Array[String]): Unit = {
    val configFilePath = Option(ParameterTool.fromArgs(args).get("config.file.path"))
    val config = configFilePath.map {
      path => ConfigFactory.parseFile(new File(path)).resolve()
    }.getOrElse(ConfigFactory.load("de-normalization-v2.conf").withFallback(ConfigFactory.systemEnvironment()))
    val denormalizationConfig = new DenormalizationConfigV2(config, "SummaryDenormalizationJobV2")
    val kafkaUtil = new FlinkKafkaConnector(denormalizationConfig)
    val task = new SummaryDenormalizationStreamTaskV2(denormalizationConfig, kafkaUtil)
    task.process()
  }
}
// $COVERAGE-ON$
