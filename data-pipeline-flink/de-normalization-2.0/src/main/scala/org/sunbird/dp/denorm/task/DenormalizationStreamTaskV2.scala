 ...
 */
class DenormalizationStreamTaskV2(config: DenormalizationConfigV2, kafkaConnector: FlinkKafkaConnector) {
  ...
  def process(): Unit = {
    ...
    val denormStream =
      env.addSource(source, config.denormalizationConsumer).uid(config.denormalizationConsumer)
        ...
        .process(new DenormalizationFunctionV2(config)).name(config.denormalizationFunction).uid(config.denormalizationFunction)
...
object DenormalizationStreamTaskV2 {
