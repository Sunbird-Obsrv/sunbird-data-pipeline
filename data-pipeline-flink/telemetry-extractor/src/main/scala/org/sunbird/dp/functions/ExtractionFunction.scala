...
class ExtractionFunction(config: ExtractionConfig)(implicit val eventTypeInfo: TypeInformation[util.Map[String, AnyRef]]) extends ProcessFunction[util.Map[String, AnyRef], util.Map[String, AnyRef]] {
   ...
   */
  lazy val extractedEventsOutPut: OutputTag[util.Map[String, AnyRef]] = OutputTag[util.Map[String, AnyRef]](id = Constants.RAW_EVENTS_OUTPUT_TAG)

  /**
   * Holding all failed events in the "failed-events" tag.
   */
  lazy val failedEventsOutPut: OutputTag[util.Map[String, AnyRef]] = OutputTag[util.Map[String, AnyRef]](id = Constants.FAILED_EVENTS_OUTPUT_TAG)

  /**
   * Holding all Log events which is generated by telemetry extractor job in the "log-events" tag.
   */
  lazy val logEventOutPut: OutputTag[util.Map[String, AnyRef]] = OutputTag[util.Map[String, AnyRef]](id = Constants.LOG_EVENTS_OUTPUT_TAG)
  ...
  override def processElement(batchEvent: util.Map[String, AnyRef], context: ProcessFunction[util.Map[String, AnyRef], util.Map[String, AnyRef]]#Context, collector: Collector[util.Map[String, AnyRef]]): Unit = {
     ...
     */
    context.output(logEventOutPut, gson.fromJson(gson.toJson(FlinkUtil.generateAudiEvents(eventsList.length, batchEvent)), (new util.HashMap[String, AnyRef]()).getClass))
  }
