...
import org.sunbird.dp.assessment.functions.AssessmentAggWindowFun
    ...
    def process(): Unit = {
      ...
      val aggregatorStream =
        env.addSource(source, config.assessmentAggConsumer).uid(config.assessmentAggConsumer)
          .rebalance()
          .keyBy(new AssessmentAggregatorKeySelector(config)).countWindow(config.thresholdBatchReadSize)
          .process(new AssessmentAggWindowFun(config))
...
class AssessmentAggregatorKeySelector(config: AssessmentAggregatorConfig) extends KeySelector[Event, Int] {
  override def getKey(in: Event): Int = {
    in.userId.hashCode % config.windowShards
  }
}
// $COVERAGE-ON$
