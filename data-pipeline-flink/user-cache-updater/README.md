...
This job is used to update the user details into redis by running the job using Flink. It can be run either on Yarn or Kubernetes.
...
```
./bin/flink run -m yarn-cluster -p 2 -yjm 1024m -ytm 1024m <data-pipeline-src-dir>/user-cache-updater/target/user-cache-updater-0.0.1.jar
```
