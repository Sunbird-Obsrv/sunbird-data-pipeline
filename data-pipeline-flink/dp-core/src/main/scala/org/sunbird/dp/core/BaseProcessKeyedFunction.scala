...
case class Metrics(counter: Map[String, Counter]) extends Serializable {

  def incCounter(metric: String) = counter(metric).inc()
...
abstract class BaseProcessKeyedFunction[T](config: BaseJobConfig) extends KeyedProcessFunction[Integer, T, T] with JobMetrics {
