...
class UserCacheUpdatetStreamTaskSpecV2 extends BaseTestSpec {
  ...
  def setupRedisTestData(jedis: Jedis) {
    ...
    jedis.hmset("user-11", EventFixture.userCacheData11)
  ...
  "UserCache Updater pipeline" should "Should able to add user data into cache" in {
    ...
    BaseMetricsReporter.gaugeMetrics(s"${userCacheConfig.jobName}.${userCacheConfig.dbReadSuccessCount}").getValue() should be(11)
    BaseMetricsReporter.gaugeMetrics(s"${userCacheConfig.jobName}.${userCacheConfig.dbReadMissCount}").getValue() should be(7)
