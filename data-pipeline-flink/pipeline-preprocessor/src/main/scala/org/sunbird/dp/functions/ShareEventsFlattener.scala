...
import org.sunbird.dp.domain.{Actor, Context, EData, Event, Object, ShareEvent}
    ...
    event.edataItems().forEach(items => {
      ...
      paramsList.getOrElse(new util.ArrayList[util.Map[String, AnyRef]]()).forEach(param => {
        ...
        val shareItemEvent = generateShareItemEvents(event,
          Object(id = identifier, ver = version, `type` = contentType, Some(Map("l1" -> event.objectID()))),
          edataType,
          size
        )
        println(new Gson().toJson(shareItemEvent))
      })
    })
  }
