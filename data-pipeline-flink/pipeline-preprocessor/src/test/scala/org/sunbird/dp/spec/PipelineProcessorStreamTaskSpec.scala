...
//  val extConfig: PipelinePreprocessorConfig = new PipelinePreprocessorConfig(config)
...
//
//    val task = new PipelinePreprocessorStreamTask(extConfig, mockKafkaUtil);
//    task.process()
//
...
//    val event2 = gson.fromJson(EventFixtures.EVENT_WITH_MID, new util.LinkedHashMap[String, AnyRef]().getClass)
//    ctx.collect(event1)
//    ctx.collect(event1)
//    ctx.collect(event2)
//  }
