...
import org.sunbird.dp.summary.denorm.functions.SummaryDeduplicationFunction
...
/**
  ...
  */
class SummaryDenormalizationStreamTask(config: SummaryDenormalizationConfig, kafkaConnector: FlinkKafkaConnector) {
...
object SummaryDenormalizationStreamTask {
  ...
  def main(args: Array[String]): Unit = {
    ...
    }.getOrElse(ConfigFactory.load("summary-denorm.conf").withFallback(ConfigFactory.systemEnvironment()))
    val denormalizationConfig = new SummaryDenormalizationConfig(config)
